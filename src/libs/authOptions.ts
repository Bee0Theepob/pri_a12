  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials: any) {
        ...
        const loginJson = await userLogIn(credentials.email, credentials.password);
  ...
  callbacks: {
    async jwt({ token, user }: any) {
      if (user) {
        ...
        token.user = { name: user.name, email: user.email, role: user.role, token: user.accessToken };
      }
      return token;
    },
